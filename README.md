...
`00040000000F5700`)의 **전체 스토리 대사 + 카드 데이터베이스 한글 패치**와, 그것을
만들기 위해 `CULDCEPT.DAT` 아카이브를 처음부터 리버스 엔지니어링한 툴 모음입니다.

오프닝부터 각 스테이지의 스토리 이벤트(13개 시나리오 컨테이너 / 약 4,500개 대화
이벤트)까지 게임 진행 중 등장하는 대화를 한국어로 표시합니다. **카드 데이터베이스
(엔트리 1190)의 카드 능력·설명·플레이버 텍스트 약 4,300개 문자열**과 시작 설정 화면
UI, 전투 HUD·카드명 라벨도 포함합니다. 한글은 **나눔스퀘어 네오 Bold**로
렌더링합니다.
...
- **한국어 번역은 `dialogue_ko.json`(대사)과 `cards_ko.json`(카드 텍스트)에 있습니다** —
  한국어만 담기며, 일본어 원문은 포함하지 않습니다(적용 시 본인 파일에서 읽음).
  카드 텍스트는 인덱스로만 매핑되고, 아이콘·제어코드 등 원문 바이트는 적용 시 본인
  파일에서 다시 읽어 보존합니다. 완성형 2350자 폰트를 쓰므로 대부분의 한글이 표시됩니다.
- **아직 번역하지 않은 화면**(캐릭터 전투 대사·일부 메뉴 등)에서는, 한글 표시를 위해
  재활용한 한자 자리에 한글 글리프가 보일 수 있습니다(완성형 방식의 알려진 특성).
