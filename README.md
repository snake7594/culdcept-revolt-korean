...
- **게임 데이터는 일절 포함되어 있지 않습니다.** ROM, `CULDCEPT.DAT`, 실행 파일,
  게임 원문 텍스트 모두 없습니다. 게임을 정식으로 소유한 상태에서 **본인의 파일**로
  사용하세요. 한글 글리프는 시스템 폰트(맑은 고딕 등)에서 그려지고, 원문 바이트는
  전부 본인의 `CULDCEPT.DAT`에서 읽어옵니다.
