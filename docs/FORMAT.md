...
- **텍스트 영역**은 null(0x00) 종료 "이벤트" 문자열의 연속입니다. 각 이벤트가 대화창 하나에
  해당합니다. 스크립트는 이벤트를 **순차 인덱스**로 참조합니다 — 헤더에 텍스트의 절대
  오프셋이 나타나지 않으므로, 이벤트 개수와 순서만 지키면 각 이벤트의 길이는 자유롭게
  바꿀 수 있습니다(포인터 재계산 불필요).
...
편집 절차: 대사 섹션을 압축 해제 → 스크립트 헤더는 그대로 두고 텍스트 영역을 새 이벤트
들로 재작성(개수·페이지 수 보존) → 0x08로 재압축 → (섹션 offset/length) 헤더를 갱신하며
컨테이너 재조립 → 컨테이너 엔트리를 파일 끝에 추가하고 레코드 갱신.
